...
class TestSafetyLimits:
    def test_default_allowed_actuators(self):
        limits = SafetyLimits()
        assert "IrrircA" in limits.allowed_actuators
        assert "VenFanrcA" in limits.allowed_actuators
        assert "CurtainrcA" in limits.allowed_actuators
...
class TestCcmSender:
    def test_send_allowed_actuator(self):
        sender = CcmSender()
        msg = sender.send("IrrircA", 1)
        assert "IrrircA" in msg
        assert "ON" in msg
    ...
    async def test_irrigation_duration_limit(self):
        ...
        with pytest.raises(ValueError, match="exceeds max"):
            await sender.send_with_duration("IrrircA", 1, 200)

    @pytest.mark.asyncio
    async def test_send_with_duration_ok(self):
        sender = CcmSender(SafetyLimits(min_send_interval_seconds=0.0))
        msg = await sender.send_with_duration("VenFanrcA", 1, 60)
        assert "VenFanrcA" in msg
